...
export class ChangeRequestRepository {
  ...
  async findActiveUser(id: string) {
    return this.db.query.users.findFirst({
      where: (user, { and, eq }) => and(eq(user.id, id), eq(user.status, 'ACTIVE')),
    });
  }
  ...
  async list(scope: ProjectScope, limit: number, offset: number) {
    return this.db
      .select()
      .from(changeRequests)
      .where(eq(changeRequests.projectId, scope.projectId))
  ...
  async create(scope: ProjectScope, submittedByUserId: string, input: CreateChangeRequestInput) {
    return this.db.transaction(async (tx) => {
      ...
      await tx
        ...
        .values({ changeRequestId: request.id, changedByUserId: submittedByUserId, newStatus: 'SUBMITTED' });
      return request;
    });
  }
}
